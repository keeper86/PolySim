...
export function getDefaultSkillsAssessment(mergeWith?: SkillsAssessmentSchema): SkillsAssessmentSchema {
    ...
    const merged = defaultSkillsAssessment.data.map((defaultCategory) => {
        const mergeCategory = mergeWith.data.find((c) => c.category === defaultCategory.category);
        if (!mergeCategory) {
...
export function cleanEmptyDefaultSkillsAssessment(schema: SkillsAssessmentSchema): SkillsAssessmentSchema {
    return {
        data: schema.data
            .map((category) => {
