        ...
        <Page title='Performance Analysis: Provenance Tracer Impact'>
            <div className='space-y-12'>
                <div className='text-lg text-slate-700 max-w-3xl'>
                    This report evaluates the performance overhead of PolySim´s file access tracing mechanism across
                    diverse workloads. Key findings:{' '}
                    <span className='font-semibold'>post-processing overhead remains low (&lt;21%)</span>, but tracing
                    overhead scales dramatically with metadata-heavy operations and small file access patterns.
                </div>
                ...
                <div className='grid md:grid-cols-2 gap-6'>
                    <div className='bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500'>
                        ...
                        <p className='text-slate-700'>
                            The cost of hashing, deduplication, and zipping traced data is consistently low across all
                            scenarios:
                            <span className='block font-mono text-sm mt-2'>
                                Mean: {postprocMeanMin.toFixed(0)}–{postprocMeanMax.toFixed(0)}% • Max:{' '}
                                {postprocStdevMax.toFixed(0)}%
                            </span>
                        </p>
                    </div>
                    ...
                    <div className='bg-white rounded-lg shadow-md p-6 border-l-4 border-orange-500'>
                        ...
                        <p className='text-slate-700'>
                            Tracing adds runtime cost that scales with syscall volume. Small files trigger high
                            overhead:
                            <span className='block font-mono text-sm mt-2'>
                                ...
                                <div className='grid md:grid-cols-2 gap-4 mb-8 bg-slate-50 rounded-lg p-6'>
                                        ...
                                        <div className='text-sm text-slate-700 space-y-1'>
                                            <p>
                                                <span className='font-mono bg-white px-2 py-1 rounded'>
                                                    {bench.stats.postprocMean.toFixed(2)}% ±{' '}
                                                    {bench.stats.postprocStdev.toFixed(2)}%
                                                </span>
                                            </p>
                                            <p className='text-xs text-slate-500'>(Cost of dedup, hash, zipping)</p>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className='text-sm text-slate-700 space-y-1'>
                                            <p>
                                                <span className='font-mono bg-white px-2 py-1 rounded'>
                                                    {bench.stats.incrTracingMean.toFixed(2)}% ±{' '}
                                                    {bench.stats.incrTracingStdev.toFixed(2)}%
                                                </span>
                                            </p>
                ...
                <div className='bg-white rounded-lg shadow-lg p-8'>
                    ...
                    <ul className='space-y-3 text-slate-700'>
                        <li className='flex items-start'>
                            ...
                            <span>
                                <strong>Sustained I/O workloads</strong> (01, 05) show low overhead (~7–170%),
                                acceptable for development and light production tracing.
                            </span>
                        </li>
                        <li className='flex items-start'>
                            <span className='text-orange-600 font-bold mr-3'>⚠</span>
                            <span>
                                <strong>Metadata-heavy operations</strong> (03: 1116%) exhibit extreme overhead. The
                                tracer captures every `open`, `stat`, `create`, making syscall-dense workloads
                                cost-prohibitive to trace.
                            </span>
                        </li>
                        <li className='flex items-start'>
                            <span className='text-blue-600 font-bold mr-3'>ℹ</span>
                            <span>
                                <strong>Post-processing is not the bottleneck.</strong> The fixed cost of archiving and
                                deduplication (1–21%) is negligible compared to tracing overhead that scales with
                                syscall volume.
                            </span>
                        </li>
                        <li className='flex items-start'>
                            ...
                            <span>
                                <strong>Recommendation:</strong> Use the tracer for development, testing, and selective
                                production use (e.g., critical workflows). For high-frequency, syscall-dense operations,
                                consider sampling or targeted tracing.
                            </span>
                        </li>
                    </ul>
                </div>
