...
exports.seed = async function (knex) {
  ...
  if (relations.wasGeneratedBy.length)
    await knex('was_generated_by').insert(relations.wasGeneratedBy);

  if (relations.used.length)
    await knex('used').insert(relations.used);

  if (relations.wasAssociatedWith.length)
    await knex('was_associated_with').insert(relations.wasAssociatedWith);

  if (relations.wasAttributedTo.length)
    await knex('was_attributed_to').insert(relations.wasAttributedTo);

  if (relations.wasInformedBy.length)
    await knex('was_informed_by').insert(relations.wasInformedBy);

  console.log(
    `Seeded ${activities.size} activities from polytrace fixtures`
  );
};
