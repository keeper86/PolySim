...
- Generated by backend, shown to the user **once**
- Stored the **hashed object** on server
- Sent via `Authorization` header
- Alternative to username/password

Note:
Unlike Keycloak, the CLI needs to be able to communicate seamlessly with authorization to the server.

---

## Why Use PATs?

**Non-interactive Authentication**: Automated systems (CLI tools, scripts) cannot perform interactive logins.

**Least-Privilege Access**: PATs can be scoped with specific permissions.

**Safe Revocation**: Compromising a PAT doesn't require resetting the user's password.

**Auditing**: Each PAT can be tracked independently (creation time, last usage)

Note:
The user gets more control over their own security 
They can choose the token to expire after some time (show later)

The PAT is set to be only read-only so it can not be changed by anyone
...
<div class="flow-horizontal">
  <div class="box fragment" data-fragment-index="1"><strong>1. Random bytes</strong><br/>64 bytes → hex</div>
  <div class="arrow fragment" data-fragment-index="2">→</div>
  <div class="box fragment" data-fragment-index="2"><strong>2. Token</strong><br/><code>a3f2...8d1c</code></div>
  <div class="arrow fragment" data-fragment-index="3">→</div>
  <div class="box fragment" data-fragment-index="3"><strong>3. HMAC</strong><br/>+ server secret <code>7c8e...</code></div>
  <div class="arrow fragment" data-fragment-index="4">→</div>
  <div class="box fragment" data-fragment-index="4"><strong>4. SHA-256</strong><br/><code>2f1a...</code> (stored in DB)</div>
</div>
...
Note:
Rainbow tables try to recreate the hashing function by looking at the other PATs  
